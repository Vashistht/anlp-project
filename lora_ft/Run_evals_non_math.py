...
def main():
    ...
    # 	# transformers.modeling_utils.load_sharded_checkpoint(model, training_args.output_dir)
        results = evaluator.simple_evaluate(
            ...
            write_out=True, 
            output_base_path='original-model_non_math',  # writes to the current dir if commented out 
        )
    ...
    if model_args.prune_info_path is not None:
        ...
        if should_i_do_eleuther_eval:
            ...
            results = evaluator.simple_evaluate(
                ...
                pretrained_model=model,
                write_out=True, 
                output_base_path=f'pruned_sparsity-{final_sparsity:.3f}_{model_args.save_info}_non_math', # writes to the current dir
            )
            updated_results = {'results': results['results']}
            print('STDOUT:', updated_results)
    ...
    if finetuned_model:
        ...
        if should_i_do_eleuther_eval:
            ...
            results = evaluator.simple_evaluate(
                ...
                pretrained_model=model,
                write_out=True, 
                output_base_path=f'finetuned_sparsity-{final_sparsity:.3f}_{model_args.save_info}_non_math', # writes to the current dir
            )
